...
var NAING_MM = "\u1014\u102D\u102F\u1004\u103A";
var NY_MM = "\u1014";
var regx_eng = /^([\d]{1,2})\/([\w]{3}|[\w]{6})\((?:N|NAING)\)([\d]{6})$/i;
var regx_mm = new RegExp('^(['+MM_NUM+']{1,2})\/(['+mmChar+']{3}|['+mmChar+']{6})[(](?:'+NY_MM+'|'+NAING_MM+')[)](['+MM_NUM+']{6})$');
...
var MMNRC = function(nrc){
  nrc = nrc.trim();
  nrc = nrc.replace(/\s/g, "");
  return new MMNRC.prototype.init(nrc);
};
...
MMNRC.prototype = {
  init : function(nrc){
    if((this.match = regx_eng.exec(nrc))){
      this.lang = "en";
      this.state = parseInt(this.match[1]);
      this.dist = this.match[2].toUpperCase();
      this.num = this.match[3];
      ...
      this.lang = "mm";
      this.state = parseInt(MMNRC.toEngNum(this.match[1]));
...
MMNRC.prototype.getFormat = function (lang){
  if(lang && lang === "mm" && !this.inCompleteInfo) {
    return MMNRC.toMyaNum(this.state) + "/" + MMNRC.revDistrict(this.dist) + "("+NAING_MM+")" + MMNRC.toMyaNum(this.num);
  } else {
...
MMNRC.prototype.getState= function (lang) {
  if (lang === "mm") {
    return states[this.state-1].mm;
  } else {
    return states[this.state-1].en;
  }
};
...
MMNRC.revDistrict = function(dist){
  var _res = "";
  for (var i = 0; i < dist.length; i+=2) {
    if(!CHARACTERS[dist[i]+dist[i+1]])
      return null;
    _res += CHARACTERS[dist[i]+dist[i+1]];
  }
  return _res;
};
...
MMNRC.formatConvert = function(nrc){
  ...
  if((_match = regx_eng.exec(nrc))){
    _res.lang = "en";
    _res.state = parseInt(_match[1]);
    _res.dist = _match[2].toUpperCase();
    _res.number = _match[3];
